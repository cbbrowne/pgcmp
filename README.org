...
*** Parameters and Inputs
     ...
     The comparison process defaults to connect to /three/ databases,
     and requires a directory in which to access input
     ...
     - Explanations File :: Optional extra file to explain expected
          differences
     - One parameter, the ACTION, which takes one of the following
       values
       - dump1 :: Dump first database's schema data, do no comparisons
       - dump2 :: Dump second database's schema data, do no comparisons
       - dump :: Dump both database's schema data, do no comparisons
       - compare :: Load data for both databases, do comparison
       - [nothing] :: Same as *compare*; load data for both databases, do comparison       
**** Environment Variables
	 ...
	 - URI1 :: URI for first database cluster
	   - If provided, the other values will be rejected
	 - FILE1 :: File where first database schema data goes
       - Second Database
	 ...
	 - URI2 :: URI for second database cluster
	   - If provided, the other values will be rejected
	 - FILE2 :: File where second database schema data goes
	 ...
	 - URIW :: URI for comparison database cluster
	   - If provided, the other values will be rejected
     ...
     - File Locations
       - WORKDIR :: Indicates a directory to use for the files used/generated in the process, defaults to ~/tmp~
